# ======================================================
# CATS & BIGCATS — FINAL (no text label, only boxes)
# ======================================================
import os, sys
    ...
    if st.session_state.mode == "Deteksi Objek":
        if STATUS["yolo_loaded"]:
            try:
                ...
                # gambar ulang bounding box tanpa teks
                for box, cls in zip(results[0].boxes.xyxy.cpu().numpy().astype(int),
                                    results[0].boxes.cls.cpu().numpy().astype(int)):
                    x1, y1, x2, y2 = box
                    color = (255, 0, 0) if cls == 1 else (0, 0, 255)  # Cats=biru, BigCats=merah
